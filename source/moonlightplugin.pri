...
dependencyList =
for(dep, plugin_deps) {
    dependencyList += "        { \"Name\" : \"$$dependencyName($$dep)\", \"Version\" : \"$$MOONLIGHT_VERSION\" }"
}
for(dep, plugin_recmds) {
    dependencyList += "        { \"Name\" : \"$$dependencyName($$dep)\", \"Version\" : \"$$MOONLIGHT_VERSION\", \"Type\" : \"optional\" }"
}
for(dep, plugin_test_deps) {
    dependencyList += "        { \"Name\" : \"$$dependencyName($$dep)\", \"Version\" : \"$$MOONLIGHT_VERSION\", \"Type\" : \"test\" }"
}
...
isEmpty(PRECOMPILED_HEADER):PRECOMPILED_HEADER = $$PWD/shared/moonlight_gui_pch.h
...
isEmpty(TARGET) {
    error("moonlightplugin.pri: You must provide a TARGET")
}
...
osx {
    ...
    QMAKE_LFLAGS += -compatibility_version $$MOONLIGHT_COMPAT_VERSION
}
include(rpath.pri)
